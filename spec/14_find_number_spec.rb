...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:chosen_number) { double('chosen_number', value: 4) }
      let(:winning_guess) { 4 }
      subject(:game_win) { described_class.new(4, 5, chosen_number, winning_guess).game_over? }
    ...
    context 'when guess and random_number are not equal' do
      let(:chosen_number) { double('chosen_number', value: 7) }
      let(:wrong_guess) { 10 }
      subject(:game_not_over) { described_class.new(1, 10, chosen_number, wrong_guess).game_over? }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'does not update max' do
        expect { low_guess_game.update_range }.not_to change { low_guess_game.max }.from(9)
      end
    end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        expect { high_guess_game.update_range }.not_to change { high_guess_game.min }.from(0)
      end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:first_edge_case) { described_class.new(5, 8, number_range, 7) }

      it 'updates min to the same value as max' do
        expect { first_edge_case.update_range }.to change { first_edge_case.min }.to eql(first_edge_case.max)
      end

      it 'does not update max' do
        expect { first_edge_case.update_range }.not_to change { first_edge_case.max }.from(8)
      end
    end

    context 'when the guess is the correct answer, when min and max are 1 off the answer' do
      subject(:second_case) { described_class.new(7, 9, number_range, 8) }

      it 'updates min to the answer' do
        expect { second_case.update_range }.to change { second_case.min }.to(8)
      end

      it 'updates max to the answer' do
        expect { second_case.update_range }.to change { second_case.max }.to(8)
      end
    end
  end
end
