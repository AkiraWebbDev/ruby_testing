...
describe Cat do
  ...
  subject(:tiggy) { described_class.new('Tiggy', 'Tuxedo', 'Black and white') }
  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'when Cat reponds to shared method name talk' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not hungry' do
    expect(tiggy).not_to be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(tiggy).to be_hiding
  end
end
