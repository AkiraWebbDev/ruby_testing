...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:choice_beverage) { described_class.new('mochaccino') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(choice_beverage.type).to be('mochaccino')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(choice_beverage.ounces).to be(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      ...
      # remove the 'x' before running this test
      it 'is full' do
        expect(choice_beverage).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:choice_beverage) { described_class.new('mochaccino', 12) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(choice_beverage).not_to be_full
      end
    end
  end
end
